...
description: >
  ...
  reconciliation. Also covers CLAUDE.md writing/auditing: "写 CLAUDE.md", "rewrite CLAUDE.md",
  "improve CLAUDE.md", "audit claude.md", "优化 CLAUDE.md", "重写 CLAUDE.md", "检查 CLAUDE.md",
  "claude.md 太长了". Also trigger when the user reports stale docs, conflicting memories,
...
### CLAUDE.md / AGENTS.md 编写原则

引用 arXiv 2507.11538：500 条指令密度下最强模型准确率仅 68%。**瓶颈不在上下文大小，在注意力分配**——指令越多遵循率越低。CLAUDE.md 控制在 **80 行以内（目标 50 行）**。

**两问法则**——每条内容必须通过，否则删：
1. **不写这条，Agent 会不会搞错？** — 能靠 `package.json`/目录结构/代码文件推断的，删。
2. **这条是不是每次会话都需要？** — 仅特定场景的，移到 `.claude/rules/` 做路径限定。

**推荐结构**（优先级从高到低）：

```
Commands → Architecture → Things That Will Bite You → Code Conventions → Don't
```

- **Commands**：构建/测试/lint 命令。Agent 推断不出工具链路径和特殊参数。
- **Architecture**：目录地图 + 数据流方向。给藏宝图，不逐模块详述（Agent 会自己读代码）。
- **Things That Will Bite You**：不看代码看不出的坑——框架 bug、编码、环境限制。
- **Code Conventions**：只写与语言/框架默认约定**不一样**的部分。默认约定不写。
- **Don't**：红线，一句话一条。不解释为什么，只要边界。

**好规则 vs 坏规则**：

| 好规则（✅） | 坏规则（❌） |
|---|---|
| "文件编码 GBK，编辑时保持原编码" | "使用 Java 17 编写代码"（pom.xml 已声明） |
| "改了命令入口 → Main.java + CliCommandParser.java + 测试" | "遵循分层架构"（目录结构已体现） |
| "构建：mvn clean package（默认跳过测试）" | "保持代码整洁"（等于没说） |
| "禁止裸跑 systemctl stop aihot-worker"（红线） | "ON 阈值 930，PENDING 阈值 875"（读代码就知道） |

**与 rules/ 目录配合**：CLAUDE.md 放最核心的 5-10 条全局规则，其余按主题拆到 `.claude/rules/`。带 `paths:` 前置字段的 `.md` 文件只在 Agent 操作匹配路径时加载，不浪费指令预算。用 `@README.md` 语法可导入外部文件。

> 日常维护：发现 Agent 没遵守的规则 → 加；发现 Agent 本来就会做对的 → 删。**像维护代码一样维护 CLAUDE.md。**
## 执行流程
...
| 文件 | Soft limit | 超过怎么办 |
|---|---|---|
| `CLAUDE.md` / `AGENTS.md` | ~80 行（目标 50 行） | 先做精简：扫顶部 blockquote / 历史叙事段 → 删 / 迁 docs；可被 Agent 读代码推断的细节 → 删；仅特定场景需要的规则 → 迁 `.claude/rules/` |
...
**CLAUDE.md / AGENTS.md 质量（中查这组）**：
- [ ] 每条规则一句话能写完，有明确的行动指导
- [ ] Commands 可以直接复制粘贴执行
- [ ] Things That Will Bite You 每个都是不看代码看不出的坑
- [ ] Don't 每一条都是明确的"禁止做 X"
- [ ] 无代码细节（阈值、常量、状态枚举值——Agent 会自己读代码）
- [ ] 无"保持代码整洁"类空话
- [ ] 可与 `.claude/rules/` 配合：仅特定场景的规则是否已迁出？
